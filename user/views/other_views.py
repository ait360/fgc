...
from ..forms.other_forms import ResendActivationEmailForm
...
class ActivateAccount(View):
    ...
    def get(self, request, uidb64, token):
        ...
        try:
            ...
            uid = urlsafe_base64_decode(uidb64).decode() #force_text(urlsafe_base64_decode(uidb64))
        ...
        if (user is not None and token_generator.check_token(user, token)):
            user.is_active = True
            user.save()
            success(request, "Your account has been activated"
                             "you may login.")
...
class LoginView(BaseLoginView):
    ...
    def get_success_url(self):
        ...
        if user.is_member and user.profile.first_name == '':
            return reverse_lazy('profile_urls:profile_update', kwargs={'username':user.username})
        elif user.is_member and not user.profile.first_name == '':
            return reverse_lazy('profile_urls:profile_detail', kwargs={'username':user.username})
